...
; Return type is void 
mergeSort:
    cmp rsi, rdx    ; compare left index and right index
    jge end         ; jump to end if left index is equal to right index
    push rsi        ; store left index(parameter 2)
    push rdx        ; store right index(parameter 3)
    add rdx, rsi    ; right index += left index
    shr rdx, 1      ; parameter 2 = (right index += left index)>>  = ((right index += left index)/=2)
    push rdx        ; store mid 
    call mergeSort  ; recurssion part 1 
    pop rsi         ; pop mid parameter2 = mid
    inc rsi         ; parameter 2 = mid++
    pop rdx         ; parameter 3 = right index
    push rdx        ; store right index 
    call mergeSort  ; recurssion part 2
    pop rdx			; parameter 3 = right index
    mov rcx, rdx	; parameter 4 = right index
	pop rsi			; parameter 2 = left index 
	add rdx, rsi 	; right index+=left index
	shr rdx, 1		; parameter 2 = (right index + left index) >>  = ((right index + left index)/=2)
    call merge      ; merge
end:
